...
**Step 1:** Spike, same shape as Task 2.2. `@react-native-documents/picker` can pick the folder but has no call that resolves its bookmark after a restart, so the folder pick and the writes live in the local module from Task 3.3 instead (`ios/CreateDocumentModule.swift`: `pickFolder()` returns the bookmark, `write(bookmark, name, content)` resolves it, opens the security scope, replaces the file atomically), and the picker package is dropped. Write `report.html` inside the folder, overwrite it shorter, force-quit, reopen, overwrite again. Check the file in iCloud Drive on a Mac or the web. Record the result under "iOS backup spike" in `docs/plans/export-headers.md`. If the folder grant does not survive the restart, use the Android shape on iOS too: `saveDocuments` twice and keep the two URLs, which is what `documents.ts` already does.

**Step 2:** In `src/backup/documents.ts`, branch on `Platform.OS`: `createDocuments(docs)` picks one folder on iOS and stores `<bookmark>#<name>` per file, or runs one create-document dialog per file on Android; `overwrite` routes to the matching native write. Settings shows "Choose backup folder" on iOS and "Set up backup" on Android; both end in the same two settings keys, so the scheduler and wiring do not change.
