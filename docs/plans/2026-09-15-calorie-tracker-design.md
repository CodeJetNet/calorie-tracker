...
4. Validate. Drop rows that fail any of: missing energy; energy above 900 kcal per 100 g; when protein, carbs, and fat are all present, `4*protein + 4*(carbs - fiber) + 2*fiber + 9*fat + 7*alcohol` (fiber and alcohol count as 0 when absent) differs from stated energy by more than the larger of 30 percent and 25 kcal; barcode fails GTIN checksum; empty name. The fiber and alcohol terms keep beer, wine, spirits, and bran cereals in the database.
