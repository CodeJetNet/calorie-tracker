...
export async function setWeight(db: Db, w: Weight, notify = true) { await db.run('INSERT INTO weights (day, kg) VALUES (?, ?) ON CONFLICT(day) DO UPDATE SET kg = excluded.kg', [w.day, w.kg]); if (notify) diaryChanged(); }
