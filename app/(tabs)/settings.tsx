...
import { DEFAULT_TARGETS, PANEL, type Nutrients } from '../../src/nutrients';
...
export default function Settings() {
  ...
  const saveGoals = async () => {   // only overrides are stored, so a later change to the defaults still reaches everyone
    ...
    for (const [k, v] of Object.entries(goal)) { const n = Number(v); if (v.trim() && Number.isFinite(n) && n !== DEFAULT_TARGETS[k]) over[k] = n; }
    await setSetting(diary, 'goals', JSON.stringify(over));
  };
  const saveMeals = async () => {
    const list = mealsText.split(',').map(s => s.trim()).filter(Boolean);
  ...
  const countries = up.manifest ? [...new Set(up.manifest.files.map(f => f.country).filter(c => c !== 'starter'))] : COUNTRIES;
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 12, gap: 8 }} keyboardShouldPersistTaps="handled">
      ...
      {up.file && (
        <>
          <Text>{installed.md5 ? 'Update available' : 'Full database available'}, {Math.round(up.file.bytes / 1e6)} MB</Text>
